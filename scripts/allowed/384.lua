...
--rot=CFrameToAxisAngle(CFrame.fromEulerAnglesXYZ(0,0,0.01))

local tick=tick
local tau=math.pi*2
local RotAxis=Vector3.new(0,1,0)
local RotSpeed=math.rad(1)*60
local Model=script.Parent
local BaseCFrame=Model:GetPivot()
game:GetService'RunService'.Heartbeat:connect(function()
	Model:PivotTo(BaseCFrame*CFrame.fromAxisAngle(RotAxis,RotSpeed*tick()%tau))
end)